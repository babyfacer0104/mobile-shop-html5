...
+function($) {
     ...
     * 调用 history.back()
     */
    Router.prototype.back = function(val) {
        var href = null;
        var close = null;
        if(val == undefined){//针对安卓手机返回处理
            href = $('.page header a:first-child').attr('href');
            close = $('.page header a:first-child').attr('onclick');
        }
        else{//触屏处理
            href = $(val).attr('href');
        }
        var cache = $(val).attr('data-no-cache') == 'true' ? true : false;
        if(navigator.userAgent.toLowerCase().indexOf('534.30') > -1 && close == null){//href.indexOf('/assets/scripts/app/list/default.html') > -1 ||
            //history.back();
            if(href == '#' || href == 'javascript:void(0)'){
                $.router.load(bp()+webUrlPath()+'/assets/scripts/app/list/default.html', false);
            }
            else{
                $.router.load(href, true);
            }
        }
        else if(close != null){
            appClose();
